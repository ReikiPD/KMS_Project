...
function RankingList({ metric, items, onOpenAsset, compact = false }) {
  ...
  return <ol className="divide-y divide-border-subtle">{items.map((item, index) => <li key={metric === "search" ? item.label || item.query : item.id} className="py-3 first:pt-0 last:pb-0"><button type="button" disabled={metric === "search"} onClick={() => metric !== "search" && onOpenAsset(item.id)} className={`flex w-full items-start gap-3 text-left ${metric === "search" ? "cursor-default" : "group"}`}><span className="kms-admin-ranking-number">{item.rank || index + 1}</span><span className="min-w-0 flex-1"><span className={`block ${compact ? "line-clamp-1" : "line-clamp-2"} text-sm font-semibold text-content-primary ${metric !== "search" ? "group-hover:underline" : ""}`}>{metric === "search" ? item.label || item.query : item.title}</span>{metric !== "search" && <span className="mt-1 block truncate text-xs text-content-secondary">{item.category_name || item.asset_type === "video" ? (item.category_name || "Video pembelajaran") : "Dokumen pengetahuan"}</span>}</span><span className="shrink-0 text-xs font-bold text-content-guide">{formatNumber(item.metric_value ?? item.search_count ?? item.view_count ?? item.share_count)}<span className="ml-1 font-medium text-content-secondary">{meta.unit}</span></span></button></li>)}</ol>;
}

function RankingCard({ metric, items, onOpenRanking, onOpenAsset }) {
  const meta = rankingMeta[metric];
  const Icon = meta.icon;
  return <CardPlain className="kms-admin-surface flex min-h-[22rem] flex-col p-5"><div className="flex items-start gap-3"><span className="kms-admin-metric-icon" aria-hidden="true"><Icon size={19} /></span><div className="min-w-0"><h2 className="text-base font-bold text-content-primary">{meta.title}</h2><p className="mt-1 text-xs leading-5 text-content-secondary">{meta.description}</p></div></div><div className="mt-5 flex-1"><RankingList metric={metric} items={items} onOpenAsset={onOpenAsset} compact /></div><Button hierarchy="tertiary" size="sm" className="mt-5 self-start" onClick={() => onOpenRanking(metric)}>Lihat peringkat lengkap</Button></CardPlain>;
}
...
function RankingModal({ metric, open, onClose, period, startDate, endDate, authorId, onOpenAsset }) {
  ...
  useEffect(() => { if (open) { setQueryInput(""); setQuery(""); setPage(1); } }, [open, metric]);
  useEffect(() => {
...
function PeriodToolbar({ period, customDates, onPeriodChange, onDatesChange, onApply, error }) {
  ...
  return <CardPlain className="kms-admin-surface mb-5 p-4 md:p-5"><div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between"><div><p className="flex items-center gap-2 text-sm font-bold text-content-primary"><CalendarDays size={17} /> Periode analitik</p><p className="mt-1 text-xs text-content-secondary">Berlaku pada KPI, tren, dan tiga peringkat dashboard.</p></div><div className="min-w-0 xl:max-w-3xl"><Chip options={PERIOD_OPTIONS} selected={customOpen ? "custom" : period} onSelect={selectPeriod} size="small" variant="outline" /></div></div>{customOpen && <div className="mt-4 grid gap-4 border-t border-border-subtle pt-4 md:grid-cols-[minmax(0,1fr)_auto]"><div><DatePicker mode="range" selected={customDates} onChange={onDatesChange} dateFormat="yyyy-MM-dd" label="Rentang tanggal" placeholder="Pilih tanggal mulai dan akhir" disabledFutureDate triggerWidth="100%" /><p className="mt-2 text-xs text-content-secondary">Rentang maksimal satu tahun hingga hari ini.</p></div><div className="flex items-end"><Button hierarchy="primary" onClick={onApply}>Terapkan</Button></div></div>}{error && <Alert className="mt-4" variant="critical" message={error} />}</CardPlain>;
}
...
export default function DashboardPage() {
  ...
  return <div className="kms-admin-dashboard mx-auto w-full max-w-7xl p-4 md:p-6 xl:p-8">
    ...
    {loading ? <DashboardSkeleton /> : dashboard && <div className="mt-5 space-y-5">
      <section aria-labelledby="organization-overview-heading"><div className="mb-4 flex flex-wrap items-end justify-between gap-2"><div><p className="kms-admin-section-eyebrow">Analitik KMS</p><h2 id="organization-overview-heading" className="mt-1 text-lg font-bold text-content-primary">{isEmployeeContext ? "Ringkasan kontribusi Pegawai" : "Ringkasan aset terbit"}</h2></div><Badge type="soft" variant="info" text={periodLabel} /></div><div className="grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-4"><MetricCard icon={BookOpenCheck} label="Pengetahuan terbit" value={organization.published_asset_count} description={periodInfo.viewMetric === "period" ? "Diterbitkan pada periode terpilih" : "Aset aktif yang dapat diakses publik"} /><MetricCard icon={Eye} label="Total dilihat" value={organization.total_view_count} description={periodInfo.viewMetric === "period" ? "Kunjungan tercatat pada periode ini" : "Akumulasi kunjungan aset terbit"} tone="teal" /><MetricCard icon={FileText} label="Dokumen" value={organization.document_count} description={periodInfo.viewMetric === "period" ? "Diterbitkan pada periode ini" : "Dokumen dan artikel terbit"} tone="gold" /><MetricCard icon={PlayCircle} label="Video" value={organization.video_count} description={periodInfo.viewMetric === "period" ? "Diterbitkan pada periode ini" : "Video pembelajaran terbit"} tone="purple" /></div></section>
      <section><CardPlain className="kms-admin-surface kms-admin-chart-card p-5"><div className="mb-4 flex flex-wrap items-start justify-between gap-3"><div><p className="flex items-center gap-2 text-base font-bold text-content-primary"><TrendingUp size={19} /> Tren Penambahan Pengetahuan</p><p className="mt-1 text-sm text-content-secondary">Aset terbit dikelompokkan {periodInfo.trendGranularity?.toLowerCase() || "bulanan"}.</p></div><Badge type="soft" variant="info" text={periodInfo.trendGranularity || "Bulanan"} /></div><TrendChart data={dashboard.publicationTrend || []} /></CardPlain></section>
