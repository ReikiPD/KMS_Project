...
import { Alert, Badge, Button, CardPlain, Divider, Skeleton } from "@idds/react";
import { Building2, CheckCircle2, Clock3, Download, Eye, FileText, ListVideo, Pencil, PlayCircle, TriangleAlert, UserRound } from "lucide-react";
...
import { API_BASE_URL, authHeaders, currentUser, uploadUrl } from "../../../../lib/api";
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        ...
        const response = await fetch(`${API_BASE_URL}/api/assets/admin/${id}/detail${query ? `?${query}` : ""}`, { headers: authHeaders(), signal: controller.signal });
      ...
      <article className="min-w-0">
        <CardPlain className="kms-admin-surface overflow-hidden">
          <div className="grid gap-0 md:grid-cols-[minmax(0,1fr)_15rem]">
            <div className="p-5 md:p-6"><div className="flex flex-wrap gap-2">{isDeleted && <Badge type="soft" variant="neutral">Terhapus</Badge>}<Badge type="soft" variant={asset.is_published ? "success" : "warning"}>{asset.is_published ? (isDeleted ? "Terbit sebelum dihapus" : "Terbit") : "Draf"}</Badge><Badge type="soft" variant="info" prefixIcon={isVideo(asset) ? <PlayCircle size={14} /> : <FileText size={14} />}>{isVideo(asset) ? "Video" : "Dokumen"}</Badge></div><h1 className="mt-4 text-2xl font-bold leading-tight text-content-primary md:text-3xl">{asset.title}</h1><div className="mt-4 flex flex-wrap gap-x-4 gap-y-2 text-xs text-content-secondary"><span className="inline-flex items-center gap-1"><Building2 size={14} />{asset.work_unit?.name || "Unit kerja belum diisi"}</span><span className="inline-flex items-center gap-1"><Eye size={14} />{asset.view_count || 0} dilihat publik</span></div></div><img className="h-full min-h-44 w-full object-cover" src={visual} alt={`Thumbnail ${asset.title}`} /></div>
          <Divider light />
